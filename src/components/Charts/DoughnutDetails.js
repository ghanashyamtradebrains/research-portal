  ...
  const hoverCenterLabel = [
    {
      ...
      afterDraw(chart, args, options) {
        ...
        if (chart._active.length > 0) {
          ...
          ctx.fillText(`${textLabel.length > 10 ? textLabel.substring(0, 20) + "..." : textLabel}`, width / 2 + 20, height / 2 + 10);
          ctx.fillText(`${numLabel.toFixed(2)}${inRuppee ? "" : "%"}`, width / 2 + 20, height / 2 + 30);
        }
      },
    },
  ];
  return (
    <div className={`flex-col-row w-100 align-items-center justify-content-center`}>
      <div style={{ height: "350px" }} className={`w-100-50 d-flex align-items-end justify-content-end ml-100-0`}>
        <DoughnutChart
          data={data}
          options={options}
      ...
      <div
        onClick={removeHover}
        className={`w-100-50 ff-rubik d-flex align-items-start justify-content-start-center`}
      >
        <div
          style={{
            overflowY: IsOverFlowHidden ? "" : "scroll ",
            maxHeight: labelsheight ? labelsheight :IsOverFlowHidden ? "100%" : "350px",
          }}
          className="scroll pointer mb-20"
        >
